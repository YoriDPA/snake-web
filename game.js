...
function getPlayerId() {
    let persistentId = localStorage.getItem('snakePlayerId');
    if (!persistentId) {
        persistentId = Math.random().toString(36).substr(2, 16);
        localStorage.setItem('snakePlayerId', persistentId);
    }
    console.log(`DEBUG [Jogo]: Meu ID Persistente é: ${persistentId}`);
    return persistentId;
}
...
function connectToServer(name) {
    ...
    ws.onopen = () => {
        console.log('DEBUG [Jogo]: Conectado! Enviando dados de join...');
        ws.send(JSON.stringify({ type: 'join', playerId: playerId, name: name }));
    };
    ws.onclose = () => console.log('DEBUG [Jogo]: Desconectado do servidor.');
    ws.onmessage = (event) => {
        // ... (O resto da função onmessage continua igual)
        try {
    ...
    ws.onerror = (error) => console.error('DEBUG [Jogo]: Erro no WebSocket!', error);
}

document.addEventListener('keydown', (e) => {
    console.log(`DEBUG [Jogo]: Tecla pressionada: ${e.key}`);
    if (!playerId || !ws) {
        console.log(`DEBUG [Jogo]: Controle bloqueado. PlayerID: ${playerId}, WS: ${ws ? 'Conectado' : 'Desconectado'}`);
        return;
    }
    let dx = 0, dy = 0;
    switch (e.key) {
...
function handleSwipe(touchEndX,touchEndY){const deltaX=touchEndX-touchStartX;const deltaY=touchEndY-touchStartY;const swipeThreshold=30;let dx=0,dy=0;if(Math.abs(deltaX)>Math.abs(deltaY)){if(Math.abs(deltaX)>swipeThreshold){dx=(deltaX>0)?1:-1;dy=0;}}else{if(Math.abs(deltaY)>swipeThreshold){dx=0;dy=(deltaY>0)?1:-1;}}
if(dx!==0||dy!==0){ws.send(JSON.stringify({type:'move',playerId:playerId,dx,dy}));}}
setInterval(draw,60);